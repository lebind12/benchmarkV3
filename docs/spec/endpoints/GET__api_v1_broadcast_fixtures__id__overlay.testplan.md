...
No test should call real API-Football or real Upstash by default. Real upstream smoke can be
added later behind an explicit manual marker.
...
At handoff time this endpoint is not implemented. Unit/integration tests are expected to fail
with missing route/module or `404` until BE starts the endpoint lifecycle.
