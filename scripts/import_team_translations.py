...
UPSERT_SQL = text(
    """
    INSERT INTO team_translation (team_id, name_ko, short_name_ko, updated_at)
    SELECT t.id, :name_ko, :short_name_ko, now()
    FROM team t
...
def main() -> None:
    if len(sys.argv) != 2:
        raise SystemExit("usage: import_team_translations.py <csv_path>")
